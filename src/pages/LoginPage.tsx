...
const LoginPage: React.FC = () => {
  ...
  const onSubmit = async (data: LoginRequest) => {
    try {
      const loggedInUser: any = await login(data);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-24 bg-obsidian-950">
      <div className="w-full max-w-[400px] animate-fade-up">
        <div className="text-center mb-10">
          <h1 className="font-display text-4xl font-medium text-gradient tracking-widest mb-2">
            LUXE
          </h1>

          <p className="font-display text-2xl font-medium text-obsidian-200">
            Welcome Back
          </p>
        ...
        <div className="card border-obsidian-700 p-7">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="flex justify-end">
              <Link
                to="/forgot-password"
                className="text-xs text-obsidian-500 hover:text-gold-400 transition-colors"
              >
                Forgot password?
              </Link>
            </div>
